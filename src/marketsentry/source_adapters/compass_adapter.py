"""Compass source adapter with dry-run parity and manual fixture support.
...
is not implemented for Compass. No network calls are performed.
"""
...
COMPASS_DOMAINS = ["www.compass.com", "compass.com"]
...
def is_compass_property_url(url: str) -> bool:
    """Check if a URL is a Compass property detail URL.

    Args:
        url: URL to check.

    Returns:
        True if URL matches Compass property pattern.
    """
    try:
        parsed = urlparse(url)
        domain = parsed.hostname or ""
        if domain not in COMPASS_DOMAINS:
            return False
        path = parsed.path.lower()
        return "/listing/" in path or "/property/" in path
    except Exception:
        return False


def is_compass_search_url(url: str) -> bool:
    """Check if a URL is a Compass search URL.

    Args:
        url: URL to check.

    Returns:
        True if URL matches Compass search pattern.
    """
    try:
        parsed = urlparse(url)
        domain = parsed.hostname or ""
        if domain not in COMPASS_DOMAINS:
            return False
        path = parsed.path.lower()
        return "/homes-for-sale/" in path or "/search/" in path
    except Exception:
        return False


def is_compass_domain(url: str) -> bool:
    """Check if URL belongs to Compass domain.

    Args:
        url: URL to check.

    Returns:
        True if URL is on a Compass domain.
    """
    try:
        parsed = urlparse(url)
        domain = parsed.hostname or ""
        return domain in COMPASS_DOMAINS
    except Exception:
        return False


def infer_compass_request_type(url: str) -> str:
    """Infer the request type from a Compass URL.
    ...
    """
    if is_compass_property_url(url):
        return "property_detail"
    if is_compass_search_url(url):
        return "search"
    return "unknown"


class CompassAdapter(SourceAdapter):
    """Compass source adapter with dry-run and manual fixture support."""
    ...
    def validate_request(self, request: RetrievalRequest) -> RetrievalResult:
        ...
        if not request.url:
            return RetrievalResult(
                source_name="compass", url=request.url,
        ...
        if not is_compass_domain(request.url):
            return RetrievalResult(
                source_name="compass", url=request.url,
                request_type=request.request_type,
                retrieval_mode=request.retrieval_mode,
                success=False, blocked=True,
                block_reason=(
                    f"URL domain is not a recognized Compass domain. "
                    f"Expected: {', '.join(COMPASS_DOMAINS)}"
                ),
                dry_run=True, network_call_performed=False,
            )

        req_type = request.request_type or infer_compass_request_type(request.url)
    ...
    def dry_run(self, request: RetrievalRequest) -> RetrievalResult:
        ...
        lines = [
            "DRY-RUN: Compass Property Detail Preview",
            f"URL: {request.url}",
            f"Domain: {domain}",
        ...
        if req_type == "property_detail":
            lines.append("Action: Would parse saved Compass property detail HTML fixture")
            lines.append("Parser: compass_parser.parse_compass_detail_html")
            lines.append("Output: Property facts, cross-site observation")
        elif req_type == "search":
            lines.append("Action: Search fixture parsing is not yet supported for Compass.")
        ...
        lines.append("")
        lines.append("Live retrieval is NOT implemented for Compass.")
    ...
    def retrieve(self, request: RetrievalRequest) -> RetrievalResult:
        ...
        self._write_audit(
            ...
            reason="Compass live retrieval is not implemented.",
        )
        ...
        return RetrievalResult(
            ...
            retrieval_mode=request.retrieval_mode,
            success=False, blocked=True,
            block_reason="Compass live retrieval is not implemented. Save page manually as HTML fixture.",
    ...
    def _write_audit(
        ...
        reason: str = "Dry-run preview (no live retrieval for Compass)",
    ) -> None:
