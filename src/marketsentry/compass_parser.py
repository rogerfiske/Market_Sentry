"""Parse Compass property detail pages from saved HTML fixtures."""
...
def parse_compass_detail_html(
    html: str, source_url: Optional[str] = None
) -> CrossSiteParseResult:
    """
    Parse a Compass property detail HTML page.
    ...
    try:
        ...
        logger.error(f"Error parsing Compass detail HTML: {e}")
...
def parse_compass_detail_file(file_path: Path) -> CrossSiteParseResult:
    """
    Parse a Compass property detail HTML file.
    ...
    try:
        ...
        return parse_compass_detail_html(html, str(file_path))
...
def _extract_displayed_dom(
    ...
    try:
        ...
        dom_match = re.search(
            r'(\d+)\s*(?:days?\s*on\s*(?:market|compass)|DOM)',
            text,
            re.I,
        )
        if dom_match:
