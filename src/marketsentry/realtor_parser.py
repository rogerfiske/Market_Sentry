"""Parse Realtor.com property detail pages from saved HTML fixtures."""
...
def parse_realtor_detail_html(
    ...
    try:
        ...
        logger.error(f"Error parsing Realtor.com detail HTML: {e}")
...
def parse_realtor_detail_file(file_path: Path) -> CrossSiteParseResult:
    """
    Parse a Realtor.com property detail HTML file.
    ...
    try:
        ...
        return parse_realtor_detail_html(html, str(file_path))
...
def _extract_displayed_dom(
    ...
    try:
        ...
        # Look for "Days on Site" or similar
        dom_match = re.search(
            r'(\d+)\s*(?:days?\s*on\s*(?:site|market|realtor)|DOM)',
            text,
            re.I,
        )
        if dom_match:
